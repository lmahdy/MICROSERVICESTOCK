...
public class DeliveryService {
    ...
    @org.springframework.beans.factory.annotation.Value("${gateway.url:http://localhost:9016}")
    ...
    public DeliveryResponse create(DeliveryRequest request) {
        ...
        if (d.getClientId() == null) {
            try {
                String url = gatewayUrl + "/api/orders/" + d.getOrderId();
    ...
    public DeliveryResponse updateStatus(Long id, DeliveryStatus status) {
        ...
        if (status == DeliveryStatus.DELIVERED) {
            ...
            try {
                String url = gatewayUrl + "/api/orders/" + d.getOrderId() + "/status/DELIVERED";
