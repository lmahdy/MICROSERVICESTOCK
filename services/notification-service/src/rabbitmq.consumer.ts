...
export class RabbitMQConsumer implements OnModuleInit, OnModuleDestroy {
  ...
  private readonly RABBITMQ_URL = process.env.RABBITMQ_URL || 'amqp://guest:guest@127.0.0.1:5672';
  ...
  private readonly DELIVERY_ASSIGNED_QUEUE = 'DELIVERY_ASSIGNED_NOTIFICATION_QUEUE';
  private readonly DELIVERY_PICKED_UP_QUEUE = 'DELIVERY_PICKED_UP_NOTIFICATION_QUEUE';
  private readonly DELIVERY_ON_THE_WAY_QUEUE = 'DELIVERY_ON_THE_WAY_NOTIFICATION_QUEUE';
  private readonly DELIVERY_DELIVERED_QUEUE = 'DELIVERY_DELIVERED_NOTIFICATION_QUEUE';
  ...
  private async connect(retryCount = 0) {
    ...
    try {
      ...
      await this.channel.assertExchange(this.DELIVERY_EVENTS_EXCHANGE, 'topic', { durable: true });

      // Create and bind queues for each delivery status
      for (const { queue, routingKey } of [
        { queue: this.DELIVERY_ASSIGNED_QUEUE, routingKey: 'delivery.ASSIGNED' },
        { queue: this.DELIVERY_PICKED_UP_QUEUE, routingKey: 'delivery.PICKED_UP' },
        { queue: this.DELIVERY_ON_THE_WAY_QUEUE, routingKey: 'delivery.ON_THE_WAY' },
        { queue: this.DELIVERY_DELIVERED_QUEUE, routingKey: 'delivery.DELIVERED' },
      ]) {
        await this.channel.assertQueue(queue, { durable: true });
        await this.channel.bindQueue(queue, this.DELIVERY_EVENTS_EXCHANGE, routingKey);
      }

      // Also keep legacy DELIVERY_DELIVERED_QUEUE for backward compat
      await this.channel.assertQueue('DELIVERY_DELIVERED_QUEUE', { durable: true });
      ...
      this.channel.consume(this.DELIVERY_ASSIGNED_QUEUE, async (msg) => {
        ...
        try {
          const event = JSON.parse(msg.content.toString());
          this.logger.log(`[RABBITMQ] DELIVERY_ASSIGNED received: orderId=${event.orderId}`);

          // Notify the CLIENT (clientId > 0 because Java sends 0 for null)
          if (event.clientId && event.clientId > 0) {
            await this.notificationService.create({
      ...
      this.channel.consume(this.DELIVERY_PICKED_UP_QUEUE, async (msg) => {
        ...
        try {
          const event = JSON.parse(msg.content.toString());
          this.logger.log(`[RABBITMQ] DELIVERY_PICKED_UP received: orderId=${event.orderId}`);
          if (event.clientId && event.clientId > 0) {
            await this.notificationService.create({
          ...
          this.logger.error('[RABBITMQ] Error processing DELIVERY_PICKED_UP', e);
          this.channel!.nack(msg, false, false);
        }
      });
      ...
      this.channel.consume(this.DELIVERY_ON_THE_WAY_QUEUE, async (msg) => {
        ...
        try {
          const event = JSON.parse(msg.content.toString());
          this.logger.log(`[RABBITMQ] DELIVERY_ON_THE_WAY received: orderId=${event.orderId}`);
          if (event.clientId && event.clientId > 0) {
            await this.notificationService.create({
              title: '🏍️ On The Way!',
          ...
          this.logger.error('[RABBITMQ] Error processing DELIVERY_ON_THE_WAY', e);
          this.channel!.nack(msg, false, false);
        }
      });
      ...
      this.channel.consume('DELIVERY_DELIVERED_QUEUE', async (msg) => {
        ...
        try {
          const event = JSON.parse(msg.content.toString());
          this.logger.log(`[RABBITMQ] LEGACY DELIVERY_DELIVERED received: orderId=${event.orderId}`);
          if (event.clientId && event.clientId > 0) {
            await this.notificationService.create({
              title: '✅ Order Delivered!',
          ...
          this.logger.error('[RABBITMQ] Error processing legacy DELIVERY_DELIVERED', e);
          this.channel!.nack(msg, false, false);
        }
      });
